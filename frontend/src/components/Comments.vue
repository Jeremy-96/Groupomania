<template>
  <div class="bloc">
    ...
    <div class="comments" v-for="(comment, commentKey) of comments" :key="commentKey">
      <div class="comment" v-if="comment.postId == postId">
        <div class="comment__infos">
          <h5 class="comment__infos__id"><em>Id : </em> {{ comment._id }} </h5>
          <h5 class="comment__infos__user"> {{ getNameComment(comment._id) }} </h5>
...
export default {
  ...
  methods: {
    ...
    getNameComment(commentId){
        let name = "";
        let id = "";
        for(let i = 0 ; i < this.comments.length ; i++) {
          if(this.comments[i]._id == commentId) {
            id = this.comments[i].userId;
          }
        }
